...
export const Pagination = ({
  ...
  const links = Object.keys([...new Array(totalNumberOfPages)]).map(
    (val) => parseInt(val) + 1
  );
  ...
  const firstLink = () => {
    if (filteredLink.some((x) => x === 1)) return null;

    const distanceEl = filteredLink.some((x) => x === 2) ? null : (
      <span css={distanceIndicatorStyle}>...</span>
    );
    return (
      <>
        <LinkComponent
          injected="true"
          className="noStyle first"
          href={createHref(1)}
  ...
  const lastLink = () => {
    ...
    if (filteredLink.some((x) => x == max)) return null;

    const distanceEl = filteredLink.some((x) => x === max - 1) ? null : (
      <span css={distanceIndicatorStyle}>...</span>
    );
    return (
      <>
        ...
        <LinkComponent
          injected="true"
          className="noStyle last"
          href={createHref(max)}
          onClick={(e) => onClick(e, max)}
          css={pageStyle}
        >
          {max}
        </LinkComponent>
  ...
  const SeoCallbackResult = seoCallback
    ? seoCallback(prevPage, nextPage)
    : null;

  const getClass = (pageNumber) => {
    if (lastPage === pageNumber) return 'last noStyle';
    if (pageNumber === 1) return 'first noStyle';
    return 'noStyle';
  };

  return (
    <nav aria-label="pagination" css={[paginationWrapperStyle, style]}>
      {isFirstPage ? (
