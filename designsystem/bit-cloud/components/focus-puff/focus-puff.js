  ...
  (
    ...
    if (!imageArea) {
      if (image) {
        ...
        if (icon) {
          imageArea = React.isValidElement(icon)
            ? React.cloneElement(icon, { style: css.puffIcon })
            : null;
        }
      }
    }
        ...
        <div css={[css.focusWrapper]} className={'focusWrapper'}>
          ...
          <div css={css.textArea} className="textArea">
            <h3 css={css.focusHeadline} className="headline">
              {headline}
              {isExternalLink && (
                <External title="Extern länk" style={css.externalIcon} />
              )}
            </h3>
            <p css={css.focusText} className="text">
              {text}
            </p>
          </div>
        </div>
